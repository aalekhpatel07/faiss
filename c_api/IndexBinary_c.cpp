...
{
    try {
        reinterpret_cast<const faiss::IndexBinary*>(index)->search(n, x, k, distances, labels);
    } CATCH_AND_HANDLE
}
...
int faiss_IndexBinary_remove_ids(FaissIndexBinary* index, const FaissIDSelector* sel, long* n_removed) {
    try {
        *n_removed = reinterpret_cast<faiss::IndexBinary*>(index)->remove_ids(
...
int faiss_IndexBinary_reconstruct_n(const FaissIndexBinary* index, idx_t i0, idx_t ni, uint8_t *recons) {
    try {
        reinterpret_cast<const faiss::IndexBinary*>(index)->reconstruct_n(i0, ni, recons);
    } CATCH_AND_HANDLE
}

int faiss_IndexBinary_search_and_reconstruct(
    const FaissIndexBinary* index, idx_t n, const uint8_t *x, idx_t k,
    int32_t *distances, idx_t *labels, uint8_t *recons)
{
    try {
        reinterpret_cast<const faiss::IndexBinary*>(index)->search_and_reconstruct(
            n, x, k, distances, labels, recons);
    } CATCH_AND_HANDLE
}

void faiss_Index_binary_display(const FaissIndex* index) {
    reinterpret_cast<const faiss::IndexBinary*>(index)->display();
}
